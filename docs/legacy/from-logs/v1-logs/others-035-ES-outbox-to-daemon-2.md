...
你自己怎么观测：三件套（指标 / 日志 / DB 快照）
1) Grafana/Prom：看“形状”而不是看单点数值

你已经有这些关键指标了（lag、oldest_age、inflight、last_success、rebuild…），它们组合起来能形成“系统心电图”。

你观测时抓 5 个问题：

是否在推进？

produced 有增长（或 outbox insert 增长）
...
last_success_timestamp 长时间不刷新
这就是 stuck 的信号。

是否在抖动/抢单？（lease 太短或 reclaim 逻辑太激进时常见）

lag 上上下下像锯齿

inflight 周期性跳变

owner mismatch skip（如果你有）升得很快

是否在“失败风暴”？（重试策略/分类错误时常见）

attempts 增长很快

pending 不断回流

429 ratio/5xx 指标上升（或者 bulk request p95 飙升）

是否“失败被隔离”？（v2 的目标之一）

deterministic error（400 mapping / parse）进入 failed

并且 lag / oldest_age 不被它拖着走

你现在最应该训练的是：每个场景跑完后，5 个指标的“形状”应该是什么样。这是比任何单元测试都更像生产的验收方式。

2) 日志：只要能回答“这条 outbox 最后怎么死/怎么活”

你不需要海量日志，你需要“可追踪日志”。最小字段：
...
claim -> process -> ack 的阶段

失败分类（retry/fail + reason）

next_retry_at（如果 retry）

lease_until（claim / heartbeat / ack 时打一次即可）
...
3) DB 快照：用 SQL 看真实状态分布

即使 Grafana 很漂亮，最后都得落回 DB 事实。

你常用 3 个查询就够了（概念上）：

状态分布：pending / processing / done / failed 各多少

stuck 扫描：processing 且 lease_until < now() 的数量

最老 pending：pending 的 oldest created_at / next_retry_at

这三条能让你在“指标看起来怪怪的”时快速确定是不是观测误差。

v1–v4 怎么验收（你说的“功能是否完善”）

我按你现在的路线，把 v1–v4拆成“可验收能力”：

v1：能跑通、不会重复写坏（claim/ack guard + 基本指标）
...
done 的 owner/lease 被清空（或符合你约定）

重启 worker 后不会把一批 processing 永久卡死（至少 lease 到期后能 reclaim）

v2：失败分类 + next_retry_at + backoff/jitter（不会失败风暴）

验收信号：

429 / 5xx：进入 pending + next_retry_at 延后，attempts 增长可控

400 mapping/parse：直接 failed，且不会反复重试拖慢队列

ES 恢复后：pending 会逐步被吃掉（不会瞬间打爆）

v3：DLQ / dead-letter（失败隔离更工程化）

验收信号：

failed 的 payload+error 被移入 DLQ 或有完整记录

主 outbox 表不会被 failed 垃圾污染得难以维护

你能手动 replay（把某条从 DLQ 重放回 pending）

v4：runbook + feature flag + rebuild（可运维）

验收信号：

你能在“不开 search projection”时让 API 正常（返回空/降级），而不是整站炸

你能一键 rebuild，并看到 rebuild 的 duration/last_success 指标更新

runbook 能指导你排障：卡死→看 inflight/oldest_age→SQL→reclaim→降级→重建

✅ 最优节奏：每推进一版，就做一次“验收回路”

也就是：

v1 完成 → 立刻跑一组场景验收 v1（10-20 分钟级别）

上 v2 → 再跑 v2 专属场景（尤其 429/5xx/400）

上 v3 → 再跑 DLQ/replay

上 v4 → 再跑降级 + rebuild + runbook 演练

原因：
daemon 的能力是“叠加的”，但 bug 也是“叠加的”。你越晚测，定位越痛苦。

给你一个很工程的做法：建一个 tests/runbook 式的自测脚本/清单（哪怕是 markdown）：

正常吞吐场景：产生 N 条事件 → lag 归零 → done 增长

重启场景：跑到一半 kill worker → lease 到期后能 reclaim → 最终 done

429 场景：限制 ES 资源或造压 → pending 回流 + next_retry_at 变大 → 恢复后逐步消化

400 场景：构造 mapping/parse 错误 → 直接 failed → lag 不被拖死

降级场景：feature flag 关 search → API 返回空/降级但服务存活

rebuild 场景：触发 rebuild → 指标记录开始/结束/耗时 → 结果可查询

你每次改完一版，只要把这套跑一遍，就知道“系统有没有退化”。

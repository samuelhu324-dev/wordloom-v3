...
## 9) 实验 6（v4）：daemon 运行时工程化（graceful shutdown / health / readiness / 告警）

目标：验证 worker 具备“可长期运维”的最小闭环：

- SIGTERM：停止 claim，新批次不再增长，并在 `OUTBOX_SHUTDOWN_GRACE_SECONDS` 内退出
...
1) 先启动 worker（见 1.4），确保端口监听
2) 验证 health/ready：

```bash
curl -fsS http://localhost:9111/healthz | cat; echo
curl -fsS http://localhost:9111/readyz  | cat; echo
```

3) 发送 SIGTERM：

```bash
pkill -TERM -f 'search_outbox_worker.py' || true
```

4) 预期：worker 在 grace 内退出；退出前不再 claim 新任务（DB 里 processing 不再扩大；最终靠 drain/release/lease reclaim 收敛）

### 9.4 判定

- `/healthz`：200
- `/readyz`：依赖 OK 时 200；draining/依赖异常时 503

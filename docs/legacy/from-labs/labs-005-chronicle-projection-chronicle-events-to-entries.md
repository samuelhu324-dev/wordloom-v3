...
完成后沉淀为 runbook：`docs/architecture/runbook/run-003-chronicle-projection.md`
...
- fixtures 条数：489
- golden 是否一致：`是`
- 观测（sync）：rebuild 后 `chronicle_entries` = 489（与 `chronicle_events` 对齐）；注意 sync 不会消费 `chronicle_outbox_events`，因此 outbox 可能仍显示 pending（属于预期现象）。
- 观测（outbox + worker）：emit outbox 后运行 worker，`chronicle_outbox_events` 最终 `done` = 489，且 `chronicle_entries` = 489。
- 判定：Projection 规则 `OK`
...
当前仓库已支持（最小形态）：通过环境变量切换 `chronicle_entries.projection_version` 写入值：

- `CHRONICLE_PROJECTION_VERSION`（优先）或 `OUTBOX_PROJECTION_VERSION`（别名）

做法（建议用 snapshot 对照，而不是“凭感觉记住”）：

1) 跑 v1 并落快照：

```bash
CHRONICLE_PROJECTION_VERSION=1 ./backend/scripts/ops_labs_005_chronicle.sh .env.test sync
```
...
3) 对照 snapshot 文件（目录：`docs/architecture/runbook/labs/_snapshots/`），确认：

- 同一批输入事件下，输出条数/排序不变
- `projection_version` 从 1 变为 2（证明“规则版本”可携带）
...
- v1（`CHRONICLE_PROJECTION_VERSION=1`）snapshot：`./_snapshots/labs-005-chronicle-sync-20260207T043714Z.txt`
- v2（`CHRONICLE_PROJECTION_VERSION=2`）snapshot：`./_snapshots/labs-005-chronicle-sync-20260207T043722Z.txt`
- 条数：两次运行 `chronicle_entries` 均为 489（输入事件集不变 → 输出条数不变）。
- 版本携带：v1 抽样行 `projection_version=1`，v2 抽样行 `projection_version=2`。
- 判定：规则版本化能力 `OK`（可通过 rebuild 重建不同版本产物，便于回滚/对照）。
...
- snapshot：`./_snapshots/labs-005-chronicle-e4-pagination-20260207T050634Z.txt`
- 数据事实：`chronicle_entries` 总数 489；`occurred_at` 存在重复（同一时间点出现 2 条的 bucket）。
- 分页稳定性：用 `(occurred_at, id)` 作为排序与游标，做两页（20+20）分页验证 `overlap_n=0`（无重复）。
- 判定：时间/分页稳定性 `OK`（必须使用 `(occurred_at, id)` 作为稳定游标）。
...
- snapshot：`./_snapshots/labs-005-chronicle-e5-failure-20260207T053909Z.txt`
- retry/backoff：`OK`（transient 注入后该行保持 `pending` 且 `next_retry_at` 有值；随后取消注入再跑一次 worker 收敛为 `done`）。
- deterministic failed：`OK`（注入非法 op：`not-a-real-op`，该行直接进入 `failed`，`error_reason=deterministic_exception` 且 error message 可用于排障）。
- max_attempts：`OK`（worker 策略：transient 只有在 `attempts >= max_attempts` 才会终态 `failed`；本实验通过调参确保能稳定展示“未耗尽 attempts → retry scheduled”。）
- replay 审计：`OK`（手工修复后对指定 outbox id replay：`failed → pending`，再跑 worker 最终 `done`；`replay_count/last_replayed_by/last_replayed_at` 均写入）。
- 判定：失败管理闭环 `OK`（transient/deterministic/replay/audit 全覆盖）。

---

## 8) 实验 6（v4）：daemon 运行时工程化（graceful shutdown / health / readiness / guardrails）

目标：把 Chronicle worker 从“脚本”升级为“可长期运维的 daemon”，验证最小闭环：

- SIGTERM：进入 draining，停止 claim 新任务，并在 `OUTBOX_SHUTDOWN_GRACE_SECONDS` 内退出
- `/healthz`：liveness（主循环在 tick）
- `/readyz`：readiness（可接活：未 draining + DB ok）
...
export OUTBOX_WORKER_ID='c-v4'
export OUTBOX_METRICS_PORT=9110
export OUTBOX_HTTP_PORT=9112
...
说明：脚本会把完整输出写入 `docs/architecture/runbook/labs/_snapshots/`。
...
```bash
python3 backend/scripts/chronicle_outbox_worker.py
```

2) 验证 health/ready：

```bash
curl -fsS http://localhost:9112/healthz | cat; echo
curl -fsS http://localhost:9112/readyz  | cat; echo
```

3) 发送 SIGTERM：

```bash
pkill -TERM -f 'chronicle_outbox_worker.py' || true
```

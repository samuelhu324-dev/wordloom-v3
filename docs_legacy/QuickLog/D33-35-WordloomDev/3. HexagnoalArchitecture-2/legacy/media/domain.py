"""
Media Domain - Global file storage system with trash and soft delete

Purpose:
- Manages Media files (Images/Videos) with centralized storage
- Supports soft delete with 30-day trash retention (POLICY-010)
- Tracks file metadata (size, mime_type, dimensions, etc.)
- Manages media-to-entity associations (Bookshelf/Book/Block can reference media)
- Enforces storage quotas and usage limits

Architecture:
- Media: AggregateRoot - represents a file uploaded to the system
- MediaPath: ValueObject - encapsulates file location and metadata
- Pure domain layer with zero infrastructure dependencies
- Emits DomainEvents on state changes

Relationships:
- Media can be associated with BOOKSHELF, BOOK, or BLOCK entities (independent associations)
- Media can be in ACTIVE or TRASH state (soft delete)
- Trash items auto-purged after 30 days (POLICY-010: Media Trash Retention)
...
class MediaUploaded(DomainEvent):
    """Emitted when a new Media file is uploaded"""
    media_id: UUID
    filename: str
...
class MediaAssociatedWithEntity(DomainEvent):
    """Emitted when Media is associated with an entity"""
    media_id: UUID
...
class MediaMovedToTrash(DomainEvent):
    """Emitted when Media is soft-deleted to Trash (POLICY-010)"""
    media_id: UUID
    occurred_at: datetime = field(default_factory=lambda: datetime.now(timezone.utc))
...
class MediaRestored(DomainEvent):
    """Emitted when Media is restored from Trash"""
    media_id: UUID
    occurred_at: datetime = field(default_factory=lambda: datetime.now(timezone.utc))
...
class MediaPurged(DomainEvent):
    """Emitted when Media is permanently deleted (30 days after trash)"""
    media_id: UUID
    occurred_at: datetime = field(default_factory=lambda: datetime.now(timezone.utc))

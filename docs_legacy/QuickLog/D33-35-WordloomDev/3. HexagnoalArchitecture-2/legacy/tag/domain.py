"""
Tag Domain - Global tagging system with multi-level hierarchies

Purpose:
- Manages global Tag catalog (independently, not nested in any entity)
- Manages Tag-to-Entity associations (TagAssociation linking Tags to Books/Bookshelves/Blocks)
- Supports hierarchical tag structures (parent-child relationships)
- Tracks usage statistics and metadata

Architecture:
- Tag: AggregateRoot - the global tag definition
- TagAssociation: ValueObject - link between a Tag and an entity (Book/Bookshelf/Block)
- Pure domain layer with zero infrastructure dependencies
- Emits DomainEvents on state changes

Relationships:
- Tag can be associated with BOOKSHELF, BOOK, or BLOCK entities (via TagAssociation)
- Tags can have parent_tag_id (multi-level hierarchy)
- NO automatic sync: Bookshelf tags ≠ Book tags (independent)
"""
...
class TagDeleted(DomainEvent):
    """Emitted when Tag is soft-deleted"""
    tag_id: UUID
    occurred_at: datetime = field(default_factory=lambda: datetime.now(timezone.utc))
...
class Tag(AggregateRoot):
    ...
    - parent_tag_id: optional UUID (for hierarchical tags)

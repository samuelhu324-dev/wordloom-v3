...
function robbery(nums: number[]): number {
    const n = nums.length;
    if (n === 0) return 0;          //  -
    if (n === 1) return nums[0]!;   // dp[0]
    
    let prev2 = nums[0]!;        // dp[i-2] -> dp[0] = 0
    let prev1 = Math.max(nums[0]!, nums[1]!); // dp[i-1] -> dp[1] = max(nums[0]!, nums[1]!)
    
    for (let i = 2; i < n; i++) {
        //  best     No Robbery       Robbery 
        //   ↓↓         ↓↓              ↓↓
...
console.log(robbery([9,7,5,3,1,8]));

...
function allSumsUnsortedBacktracking(nums: number[], target: number): number[][] {
  ...
  function backtrack(start: number, sum: number) {
    for (let i = start; i < nums.length; i++) {
      ...
      // dive into the branch, say, [1] -> [1,2] -> [1,2,3] (if index 3 is max)
      // then pop the path from [1,2,3] -> [1,2] -> [1] 
      // (assume that all indices are valid)
...
console.log(allSumsUnsortedBacktracking([2,3,4,4], 5));

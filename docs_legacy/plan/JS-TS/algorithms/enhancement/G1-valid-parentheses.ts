...
//    from the top of the sack and compare
...
function isValidParentheses2(s: string): boolean {
    ...
    for (const ch of s) {
        if (ch in map) {
            // if ch is '[', we push ']' onto the stack
            stack.push(map[ch]!);
        } else {
            // if ] - ) in order, we pop ')' from the stack bottom
            // see an example: [()]
            // push: ] - ) / expected: ) - ]
            const expected = stack.pop();
            if (expected !== ch) return false;
...
console.log(isValidParentheses2('()[]{}'));   // true
console.log(isValidParentheses2('()[]}'));    // false
console.log(isValidParentheses2('(([]{}'));   // false
console.log(isValidParentheses2(''));         // true（按常规定义）
console.log(isValidParentheses2('()[]()'));   // true
console.log(isValidParentheses2('([{}])'));   // true
console.log(isValidParentheses2('(]'));       // false

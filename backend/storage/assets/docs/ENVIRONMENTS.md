...
- 启动 Elasticsearch（以及可选监控）：
	- PowerShell：`./backend/scripts/run_infra.ps1`（或加 `-Monitoring`）

在 WSL2 / bash：

- 启动 dev API：`./backend/scripts/run_api.sh .env.dev`
- 启动 dev worker：`./backend/scripts/run_worker.sh .env.dev`
- 启动 test API：`./backend/scripts/run_api.sh .env.test`
- 启动 test worker：`./backend/scripts/run_worker.sh .env.test`

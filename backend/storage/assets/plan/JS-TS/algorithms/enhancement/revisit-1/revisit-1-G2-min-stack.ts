...
class MinStack {
    ...
    // there are two stacks needed to maintain:
    // 1. normal stack; 2. minStack
    push(x: number): void {
        this.stack.push(x); 
...
function createMinStack() {
    ...
    return {
        ...
        push(x: number): void {
            stack.push(x);
            if (!minStack.length || x <= minStack[minStack.length - 1]!) {
                minStack.push(x);
            }
        },
        ...
        pop(): void {
            ...
            const v = stack.pop()!; // [5]
            if (minStack.length && v === minStack[minStack.length - 1]!) {
                minStack.pop();     
            }
        },
        ...
        getMin(): number | undefined {
            if (minStack.length) return minStack[minStack.length - 1]!
            return undefined;
        },
    };
}
...
s1.push(7);  // [5,7] <-> [5]
s1.push(6);  // [5,7,6] <-> [5]
console.log(s1.top());    // 6
console.log(s1.getMin()); // 5
s1.pop();    // [5,7] <-> [5]
console.log(s1.getMin()); // 5

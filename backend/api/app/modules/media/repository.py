"""Media Repository - Data persistence layer

Architecture (ADR-026: Media Repository Design):
================================================
- Abstract repository interface (MediaRepository ABC)
- Concrete implementation using SQLAlchemy (SQLAlchemyMediaRepository)
- Query methods for common access patterns:
  * save(media) - persist uploaded media
  * get_by_id(media_id) - fetch single media
  * delete(media_id) - move to trash (soft delete)
  * restore(media_id) - restore from trash
  * purge(media_id) - hard delete (after 30 days)
  * find_by_entity(entity_type, entity_id) - media on specific entity
  * find_in_trash(skip, limit) - paginated trash listing
  * find_active(skip, limit) - paginated active media
  * count_in_trash() - trash count for UI
  * find_eligible_for_purge() - 30-day retention check
  * associate_media_with_entity() - create association
  * disassociate_media_from_entity() - remove association
- State-based filtering: state (ACTIVE|TRASH) + trash_at timestamp
...
from domain import Media, MediaState, EntityTypeForMedia
from models import MediaModel, MediaAssociationModel, MediaState as ModelMediaState
...
class MediaRepository(ABC):
    """Abstract repository for Media persistence"""
...
class SQLAlchemyMediaRepository(MediaRepository):
    ...
    def _model_to_domain(self, model: MediaModel) -> Media:
        ...
        from domain import MediaMimeType
        return Media(
            id=model.id,

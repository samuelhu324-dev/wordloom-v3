"""HardDeleteBookUseCase implementation."""
...
from ..dtos import HardDeleteBookCommand


class HardDeleteBookUseCase:
    """Permanently remove a book that currently lives inside Basement."""
    ...
    async def execute(self, command: HardDeleteBookCommand) -> None:
        ...
        try:
            timestamp = command.deleted_at or datetime.now(timezone.utc)
            book.mark_deleted(timestamp)
            await self._book_repository.save(book)
            await self._publish_events(book)

            # Remove BasementEntry before hard-deleting the book
            await self._basement_repository.delete_by_book_id(command.book_id)

            await self._book_repository.delete(book.id)
        except (BookNotInBasementError, BookOperationError):
            raise
        except Exception as exc:  # pragma: no cover - defensive guard
            raise BookOperationError(f"Failed to hard-delete book: {exc}") from exc

...
from modules.block.domain import Block, BlockType, BlockContent, BlockTitle
...
def block_model_factory(sample_book_id):
    ...
    def _create(
        ...
        from modules.block.models import BlockModel, BlockType as DBBlockType
        now = datetime.now(timezone.utc)
        return BlockModel(
            id=block_id or uuid4(),
            book_id=book_id or sample_book_id,
...
async def assert_heading_level_required():
    ...
    async def _verify(repository):
        from modules.block.models import BlockModel, BlockType

...
\restrict JhgQfJFgd016X2rsj5V8FQOTOwAcxh0ujlJUWZfLPwJ8c8P95TRRHnL0PvRbMKU
...
CREATE TABLE public.entries (
    ...
    lang_src character varying(8),
    lang_tgt character varying(8),
    created_at timestamp without time zone,
    article_id integer,
    "position" integer
);
...
--
-- Name: entry_sources; Type: TABLE; Schema: public; Owner: -
--
...
COPY public.articles (id, title, source_id, created_at) FROM stdin;
\.
...
COPY public.entries (id, src_text, tgt_text, lang_src, lang_tgt, created_at, article_id, "position") FROM stdin;
1	We can use [memory forensics] to identify malware that may have been only [resident in] memory and never saved on disk. We can also use it for evidence as to the activities of a user at a given time.	我们可以使用[内存取证]去识别可能一直只[寄存于]内存，从不在硬盘上保存的恶意软件。我们也可以使用内存取证找关于在一个给定时间点一名用户活动的证据。	en	zh	2025-10-02 14:04:16	\N	\N
2	We support [electronic discovery] for civil [litigators] by collecting traditional digital documents and email evidence from your client or [opposing party].	我们收集你的客户或者[反对方]的传统数字文档和电邮证据，就可以支持民事[诉讼律师][电子举证]。	en	zh	2025-10-04 02:25:44	\N	\N
3	One case for the extension premise comes from advances in information technology	其中一个延展假定的情况出自信息技术方面的进步	en	zh	2025-10-06 01:40:14	\N	\N
4	Whenever we [come up with] a computational product, that product [is soon afterwards obsolete] due to technological advances	每当我们[拿出]一个计算机上的产品，那个产品由于科技进步[不久之后就废弃了]	en	zh	2025-10-06 01:40:14	\N	\N
5	We should expect [the same]to apply to AI	我们应该希望[同样的情况]适用于AI	en	zh	2025-10-06 01:40:14	\N	\N
6	Soon after we have produced a human-level AI, we will produce an even more intelligent AI	不久后我们已经产出人类级别的AI了，我们就将产出一个更加聪明的AI：一个AI+	en	zh	2025-10-06 01:40:14	\N	\N
7	Please [consult] the solution [given] for phase 2 of the project before attempting this quiz;\r\nthe solution can be found on LMS.	在尝试这个小测前，请[查阅]项目阶段2[给出的]解决方案；解决方案在LMS上可以找到。	en	zh	2025-10-06 09:50:30	\N	\N
8	Select all appointment numbers (apptNo) from the Appointment table, and the dateAndStartingTime (display as 'appointment data and time'). Only select appointments that are on the 24th or the 26th day [of month].\r\nHint: you should use the strftime() function.	选中Appointment表格里的所有问诊编号（apptNo）和dateAndStartingTime（显示为'问诊数据和时间'）只选中在[每月]24日或者26日的问诊。\r\n提示：你应该使用strftime()函数。	en	zh	2025-10-06 10:01:08	\N	\N
9	Select the uniqueIdentifierCode, dateAndStartingTime, clientNumber, prescribedCode, description, and currentServiceFee from all of the required tables, utilizing a JOIN.\r\nHint 1 which of the above attributes are keys that are common between tables, and which are only found in specific tables? You will need to differentiate those that appear in multiple tables that you join.\r\nHint 2 You will need to use 4 tables.	运用了一次JOIN，就能选中所有需要的表格里的uniqueIdentifierCode、dateAndStartingTime、clientNumber、prescribedCode、description和currentServiceFee，。\r\nHint 1 以上的属性哪个是表格之间很常见的钥匙，而哪些只在具体的表格中找到。\r\nHint 2 你得要使用上4个表格。	en	zh	2025-10-06 10:34:17	\N	\N
10	Select the title, personalName, familyName from the Doctor table and order by personalName in ascending order, and by familyName in descending order.	选中Doctor表格里的title、personalName、familyName，以升序order by（按排序）personalName，并以降序排序familyName。	en	zh	2025-10-06 11:18:05	\N	\N
11	Count the total number of services offered (display as 'Total Services'), and calculate the average and total service fees (display as 'Avg. Service Fee' and 'Total Service Fee'), from the Services table.	Count已提供服务的总数字（显示为'Total Services'）并算出average和total服务费用（显示为'Avg. Service Fee'和'Total Service Fee'）在Services表格里数和算。	en	zh	2025-10-06 11:23:03	\N	\N
12	[For] each Doctor, find how many services they offer, and display the Doctor’s personalName and familyName, as well as the count of services offered (displayed as 'Service Count').\r\nOnly [show] doctors who offer more than 2 services.\r\nHint: You will need to use the tables Doctor and Gives.\r\nHint 2: having [will be useful].	[算]每个医生，求出他们提供多少服务并显示Doctor的personalName和familyName，以及已提供服务的个数（显示为'Service Count'）。\r\n只[写]提供两个以上服务的医生。\r\n提示：你得要使用上表格Doctor和Gives。\r\n提示2：having[会很有用]。	en	zh	2025-10-06 12:12:40	\N	\N
13	Select the names of doctors who give a service that costs more than $160.\r\nHint: you will need to use sub queries	选中给出一个花费高于160刀的服务的医生的名称。\r\n线索：你得要使用上子查询。	en	zh	2025-10-06 13:57:15	\N	\N
14	Select the personalName and familyName of patients who have an appointment with a doctor that works in Room 4.\r\nHint: You can use sub queries or WHERE or JOIN.	选中与一名在Room 4工作的一位doctor有一次appointment的病人的personalName和familyName。\r\n线索：你可以使用子查询或者WHERE或者JOIN。	en	zh	2025-10-06 14:14:30	\N	\N
15	Create a view named CompletedInvoices that shows the uniqueInvoiceNumber, apptNo, relevantDate, and status, [for only completed ('C') invoices].	创建一个命名为CompletedInvoices的一张view，写上[只有填完的（'C'）发票才适用的]uniqueInvoiceNumber, apptNo, relevantDate, and status。	en	zh	2025-10-06 14:44:27	\N	\N
16	Select all patient's clientNumber who have a Patient record but are NOT listed as a ResponsibleCustomer.	选中有一次Patient记录但没有列作一名ResponsibleCustomer的所有病人的clientNumber。	en	zh	2025-10-06 14:51:52	\N	\N
17	Select all appointments apptNo, dataAndStartingTime and uniqueNumber (as 'Room Number') that happened between '2025-04-24' and '2025-04-27' AND where the room number [is either] 2, 3, [or] 5.\r\nHint: you [will need to use] date() function.	选中在'2025-04-24' 和 '2025-04-27'之间进行AND其中房间编号[不是]2、[就是]3[就是]5的所有问诊apptNo、dataAndStartingTime和uniqueNumber（选为'Room Number'）\r\n提示：你[得要使用上]date()函数。	en	zh	2025-10-06 20:42:41	\N	\N
18	[AI] A VIEW in SQL is like a virtual table - it doesn’t store data itself, but it [shows] data that comes from one or more real tables through a saved query	SQL中一次VIEW像是一张虚拟表，表没储存数据本身，但表[上面有]透过一个保存的查询来自一个或多个真实表格的数据	en	zh	2025-10-06 21:41:10	\N	\N
19	[AI] You can think of it as a stored SELECT statement [with a name]	你可以把表想成是一个[有名字的]、SELECT储存语句	en	zh	2025-10-06 21:41:10	\N	\N
20	[AI] The database will then run the original query [behind the scenes], [pulling] the latest data from the underlying tables	数据库然后将[拉出]底层表格里的最新数据，就能[在幕后]运行原始查询	en	zh	2025-10-06 21:41:10	\N	\N
21	This lab will be using [live] malware samples	这个Lab将会使用[实时]恶意软件样本	en	zh	2025-10-07 10:33:30	2	1
22	Although the samples we use [are not capable of] [breaking out] of docker containers, it is best [to minimize the risk] by doing this lab within a virtual machine (VM), such as using VirtualBox with Ubuntu or Kali Linux	尽管我们使用的样本[没有能力][打破]docker匣子，但最好在虚拟机之中做这个Lab（譬如说用Ubuntu或者Kali Linux使用VirtualBox）就可以[把风险降到最低]	en	zh	2025-10-07 10:33:30	2	4
23	Please check the Setting up VM for labs sections	你在你的主机机械里直接在做Lab的话，一旦你已经做完Lab去移除所有恶意样本了，你应该删除匣子，否则你的杀毒软件可能很不高兴并发脾气	en	zh	2025-10-07 10:33:30	2	2
24	If you are doing the lab directly from your host machine, you should delete the container once your have finished the lab to remove all malware samples - otherwise your antivirus software may not be happy and throw a tantrum	你运行匣子的时候可以添加--rm旗子就可以自动地删除匣子	en	zh	2025-10-07 10:33:30	2	3
25	Artificial intelligence is [a collection of] technologies that allows computers to [simulate] human intelligence	人工智能是让电脑可以[模拟]人类智能的[一组]科技	en	zh	2025-10-07 11:26:14	2	5
26	Its applicability in cybersecurity involves [all aspects of] AI, including natural language processing, [speech recognition], expert systems, robotics and vision	在网络安全中智能的适用性涉及AI的[方方面面]，包括自然语言处理、[语音识别]、专家系统、机器人与视觉	en	zh	2025-10-07 11:26:14	2	6
27	[Fundamental to these types of AI is] machine learning, a technology that uses an approach to learning that tries and mimics the way [nerve cells] work in the brain	[对这些类型的AI密不可分的是]机械学习，是使用尽可能模仿大脑中[神经细胞]怎么工作的一类学习措施的一种科技	en	zh	2025-10-07 11:26:14	2	7
28	One area of [active] research in machine learning is the use of [adversarial images]	机械学习的[热门]研究，其中一个领域是对[对抗图片]的使用	en	zh	2025-10-07 12:49:52	2	8
29	This is where slight changes in the image causes machine learning systems to incorrectly classify objects within the image	这是轻轻在图片上改在图像内就造成机械学习系统去不正确地分类对象	en	zh	2025-10-07 12:49:52	2	9
30	So for example, a panda is wrongly recognised as a [gibbon]	所以比如说，一只熊猫错误地识别为一只[长臂猿]	en	zh	2025-10-07 12:49:52	2	10
31	[The changes to the image can be imperceptible to a human] and so this type of attack could be used to alter a [radiological image] and change [a negative diagnosis of cancer] to a positive one in a system that is automatically [screening for] cancer	[图像的更改一名人类可以察觉不到]，并且这类型的攻击能够使用去改动一张[放射图片]，在自动地[筛查]癌症的系统中把[对癌症的一次阴性诊断]改为一次阳性诊断	en	zh	2025-10-07 12:49:52	2	11
32	Adversarial attacks [could] become increasingly common as we [come to] rely on machine learning systems to automate processes and decisions	由于我们[去]依赖机械学习系统去自动化进程和决策，对抗攻击[说不定]变得日益常见	en	zh	2025-10-07 16:38:01	2	12
33	This [has made] the field of [research into] [defences against this type of attack] [important]	这[使得][对于][防御这类型攻击][的研究]领域[变得重要起来]	en	zh	2025-10-07 16:38:01	2	13
34	In the area of malware recognition, malware can adopt tactics to prevent from recognition by taking an adversarial approach	在恶意软件识别领域，恶意软件可以采取对抗措施，就可以采用策略去阻止识别	en	zh	2025-10-07 16:38:01	2	14
35	To [see how this works], we can use a program that uses what is called a non-targeted black box approach to adversarial images	要[懂这怎么用]，我们可以使用使用解决对抗图像所谓的非目标黑盒子措施的一个程序	en	zh	2025-10-07 16:38:01	2	15
36	To set it up, follow the instructions below	要把程序安装上，按照以下的指示	en	zh	2025-10-07 16:38:01	2	16
37	Also install matplotlib inside the container to run the code later	并安装匣子里面的matplotlib去运行之后的代码：一旦在docker匣子上，[还没有]转的话就转到目录/opt	en	zh	2025-10-07 16:42:45	2	17
38	Once on the docker container, go to the directory /opt if [not already]	从那里运行exploit	en	zh	2025-10-07 16:42:45	2	18
39	From there, run the exploit	py程序	en	zh	2025-10-07 16:42:45	2	19
40	py program	这个程序将摄取一只拉布拉多一张正常的图像并创建对抗版的图像	en	zh	2025-10-07 16:42:45	2	20
41	This program will take a normal image of a Labrador and create adversarial versions of the image	你对详细内容有兴趣的话，程序就来自一个叫做Foolbox的工具箱里	en	zh	2025-10-07 16:42:45	2	21
42	To run the script, we [do so as follows]	要运行脚本，我们[如下这样做]：	en	zh	2025-10-07 16:45:18	2	22
43	This takes the initial image of lab_og.jpg and then creates different versions of the image with the [perturbations] added. The script tests these images against the neural network and very quickly stops recognising a [Labrador] and starts recognising other dog types such as the [Saluki] and [Welmaraner], although note that [the levels of confidence] in that result are very low [~] 16%.s	这脚本取lab_og.jpg起初的图像，然后用已添加的[扰动]创建不同版本的图像。脚本针对神经网络测试这些图像并非常快就停止识别一只[拉布拉多]并开始识别其他狗类型，譬如[萨卢基]和[魏玛]，尽管注意到那个结果的[置信水平]非常低，[约]16%，[脚本也会识别]。	en	zh	2025-10-07 17:00:31	2	23
44	After running the program, you should have some files	运行程序之后，你应该有一些文件：	en	zh	2025-10-07 17:07:50	2	24
45	Copy these files to your host VM/machine (open up another terminal and use docker cp command)	把这些文件拷贝到你的主机VM/机械（再打开一个终端并使用docker cp命令）	en	zh	2025-10-07 17:09:03	2	25
46	Note the Wolfram site [only works] [intermittently] - it isn't reliable. If you can't get it to work, don't worry - just use the [predictions] that the program exploit.py printed out.	注意Wolfram网址[只能][断断续续][工作] - 不是很可靠。你不能让网址工作的话，就别担心 - 使用程序exploit.py打印出了的[预测值]就行了。	en	zh	2025-10-07 17:18:32	2	26
47	Load the first image, Input.jpg into the site and verify that it is correctly recognised. Then try with [each of the other] adversarial images [starting with] Epsilon = 0.010.jpg and [going up]. If the site [errors out], wait a bit and then try again, it seems to not like images being loaded too quickly. Eventually, it should fail to recognise the last image (or [misclassify]) which [has had the most] perturbations applied to it.	把第一张图像Input.jpg加载进网址并核实图得到正确地识别。然后试试用[其他的每个以Epsilon = 0.010.jpg开头]的对抗图像并[升一升级]。网址[有错]的话，就等等然后再试一次，网址似乎不喜欢图片非常快被加载。最终，网址无法识别最后一张图（或者[错分]）已施加给图的扰动[是最多的]。	en	zh	2025-10-07 18:01:40	2	27
48	Ideally Wolfram should produce the same prediction, but because its model [is continuously updating], the prediction may not be [the same].	理想情况下，Wolfram应该得出同一个预测值，但是因为程序的模型[不停在更新]，预测值可能不是[同一个值]。	en	zh	2025-10-07 18:05:53	2	28
49	[To a human eye], the dog is still a Labrador albeit a bit [fuzzy]. Remember that this is not a [targeted] attack [in that] we [haven't] trained the attack [using] the specific neural network used by the Wolfram site. If we did, we could develop something that would work at much lower levels of disturbance.	[人类的眼里]，狗纵使有点[糊]仍然是一只拉布拉多。记得这不是一个[针对性的]攻击，[这一点上]，我们[尚未]使用Wolfram网址已使用的具体神经网络，[就不能]训练攻击。我们训练了的话，我们就能够开发会以更低扰动水平工作的某个东西。	en	zh	2025-10-07 18:16:17	2	29
50	Try the same attack but this time using another picture - one of a bear:	试试同一个攻击，但这次再使用一张图，其中一只熊的图就能攻击：	en	zh	2025-10-07 18:17:26	2	30
51	You can right click on this and save it to your share folder and then run the program again. You might see [the confidence dropping], but the prediction should still be the same - a brown bear. Obviously, not all photos/pictures [are well equipped for] the perturbations applied using the provided [exploit code], but there are many other libraries available that perform better perturbations.	你可以右键点击这张图并把图保存到你的分享文件夹，然后再运行程序。你可能会见到[置信值在下降]，但预测值应该仍然是同一个值 - 一只熊。很显然，不是所有的照片/图片都使用提供的[挖洞代码]就能[预备好]已施加的扰动，但有很多其他完成更好的扰动用得到的库。	en	zh	2025-10-07 18:33:32	2	31
52	Try with your own images - note that they need to be [the appropriate size of 700px wide] so [scale them down] by resizing in photo editing software if they are larger.	用你自己的图像试试 - 注意图得要是[合适的大小为700px宽]，所以图比较大的话，在照片编辑软件[调整大小]就能[缩小图的比例]。	en	zh	2025-10-07 18:36:27	2	32
53	Detecting malware relies on a static analysis of features such as the hash of the file and its use of strings (as we saw when we used Yara to identify malware). This is fine if you encounter malware that is something you have in a database of malware samples. Machine learning however tries to identify new malware by analysing the features and using a model that has been trained on millions of previous samples to be able to answer the question of whether the program is malware or not, rather than identifying the specific type of malware.	检测恶意软件依赖对特征的静态分析，譬如（我们使用了Yara去认出恶意软件的时候我们所看到的）文件的哈希码和码对字符串的使用。你遇到是你在一个恶意软件样本数据库中有的某个东西的恶意软件的话，这就还好。机械学习不过就使用一下已经在数百万之前的样本上训练出来回答得出程序是恶意软件与否，而非识别具体类型的恶意软件的问题的一个模型，就能认出新的恶意软件。	en	zh	2025-10-07 20:16:48	2	33
54	The Elastic Malware Benchmark for Empowering Researchers (Ember) (https://github.com/elastic/ember) is a neural network that uses features extracted from binary files to train a model that can distinguish malware from regular Windows' programs.	授权研究人员弹性恶意软件基准（Ember）（https://github.com/elastic/ember) 是使用二进制里已提取的特征去训练可以在常规Windows程序里辨别恶意软件的一个模型的一个神经网络。	en	zh	2025-10-07 20:20:15	2	34
55	To do this, Ember uses a framework called LIFE that will analyse Windows (and other platforms) [binaries] (https://github.com/lief-project/LIEF). The data that LIFE produces [then] uses a fingerprint of the binary [and] can be used to train the model. Let's start the docker container to run Ember:	要做到这一点，Ember使用将分析Windows（和其他平台）[二进制]（https://github.com/lief-project/LIEF） 的一个叫做LIFE的框架。LIFE产出的数据[然后就]使用二进制的一个“指纹”[就]可以使用来训练模型。我们来开启docker匣子去运行Ember：	en	zh	2025-10-07 20:30:32	2	35
56	To test the model cd on the same docker container to the directory /opt/ember. The model has been pre-trained and so you don't need to do that, although if you are interested, you can (look at the GitHub page).	要测试模型，在同一个docker匣子上cd到目录/opt/ember。模型已经预训练，并且你不用那么做，尽管你有兴趣的话你就可以（看GitHub页面）去预训练。	en	zh	2025-10-07 22:45:20	2	36
57	First, unzip the malware (with the usual password: infected):	首先，（用通常的密码：infected）拉开恶意软件。	en	zh	2025-10-07 22:46:09	2	37
58	To test the malware, we can use the following command:	要测试恶意软件，我们可以使用如下指令：	en	zh	2025-10-07 22:46:31	2	38
59	The script classify_binary.py takes the trained model as an argument and the binary file to analyse. As you can see, the sample malware is given a >99% [probability] [of being malware].	脚本classify_binary.py把训练模型取为变量和二进制文件去分析。可见，样本恶意软件给到了>99%的[概率][是恶意软件]。	en	zh	2025-10-07 22:52:11	2	39
60	This *is* [real] malware - [do] not download to your PC [or try] and execute.	这是[真实存在的]恶意软件不要下载到你的PC[或者尽量不要]执行。	en	zh	2025-10-07 22:52:51	2	40
61	The malware is actually Trickbot, which is a [banking trojan].We can now try with a normal Windows program git.exe.	恶意软件实际上是Trickbot，是一个[银行木马]。我们现在就可以试试用一个正常的Windows程序git.exe。	en	zh	2025-10-07 22:55:41	2	41
62	What is returned is a *very* small number 1.7*10-7. In other words, a [practically] 0 score [for it being malware].	返回的是一个*非常*小的数字1.7*10-7。换句话说，[测出来是恶意软件][简直]0分	en	zh	2025-10-07 23:00:16	2	42
63	Msfvenom is a utility that comes with what is called an exploitation framework called Metasploit. Metasploit is a [sophisticated] [toolset] that is used by penetration testers (and hackers) to explore vulnerabilities and exploit them. Msfvenom can generate various [payloads] that when run on a target machine, will create remote access to that machine for attackers. One of these payloads is called a Meterpreter shell and normally this is recognised as malware by anti-malware software.	Msfvenom是自带所谓叫做Metasploit的挖洞框架的一个协程。Metasploit是被渗透测试员（和黑客）是用来探究并利用漏洞的一个[高精密][工具套组]。Msfvenom可以生成在一个目标机械上运行时就可以给攻击者创造那个机械的远程访问权的各种[装载码]。这些装载物的其中一个叫做Meterpreter shell，而正常情况下这个shell被杀毒软件识别为恶意软件。	zh	en	2025-10-08 17:57:47	2	43
64	Msfvenom comes with [a variety of] evasion techniques to avoid detection but most are easily detected by anti-malware software. However, there exists one approach that [does fool] anti-malware, especially machine learning classifiers.	Msfvenom自带[各种各样的]回避技巧去规避检测，但大多数都很容易被杀毒软件检测到。不过存在一个[糊弄到]恶意软件的措施，尤其是机械学习分类器。	en	zh	2025-10-08 17:58:47	2	44
65	[To get started], we are going to run Metasploit. But firstly, we will create a [local volume] [that we can attach to different containers] - this is due to the files we are about to create will most likely [be filtered] automatically by your firewall. So let's create a temporary volume. In your PowerShell/terminal:	[一开始的话]，我们要运行Metasploit。但首先，我们将创建[我们可以附在不同匣子上的]一个[本地卷] - 这主要是我们即将创建的文件有可能将你的防火墙被自动[筛掉]。所以我们来创建一个暂时的卷。在你的PowerShell/terminal中：	en	zh	2025-10-08 17:58:58	2	45
66	This will create a local volume named volume1, you can check by docker volume ls. Now we can attach this volume instead of host's [local drive] and share files between containers. Let's run a Docker container as follows:	这将创建一个名为volume1的本地卷，你可以拿docker volume ls来检查。好了我们可以附加这个卷，而不是主机的[本地驱动器]，并在匣子之间分享文件。我们来运行如下的Docker匣子：	en	zh	2025-10-08 20:42:36	2	46
67	This will create a folder called volume1 in the root directory (you can name this [something else] e.g., volume1:/extra), which is attached to the local volume1.	这将在root目录（你可以把这个目录叫做[别的什么]，如volume:/extra）创建一个叫做volume1的文件夹。	en	zh	2025-10-08 20:42:46	2	47
68	Then, we [are going to need] a file called putty.exe that we are going to use as a template for one of the Meterpreter versions. If you have a [shared] folder between the container and the host, download it by going to the address below and save it to the directory you have shared with the container (you can paste the link in the browser to download, and save it into the shared folder:)	然后，我们[得打算要]一个叫做putty.exe的文件\r\n我们打算使用为其中一个Meterpreter版本的一个模板。你在匣子和主机之间有一个[共同的]文件夹的话，就转到以下的地址就可以下载并把文件保存到你与匣子已经共享的目录（你可以粘贴浏览器中的链接去下载并把文件保存进共用的文件夹中：）	en	zh	2025-10-08 20:57:03	2	48
69	Alternatively (e.g., using the shared local volume approach), you can use wget to download directly from docker (and move it into the shared folder). You would need to apt-get update and apt-get install wget on the container.Now, we can run Metasploit by typing: msfconsole. If you are asked questions, answer yes to the first question (set up a new database) and no to the second ([init] the web service). After doing that, you should see [something like this] (the actual graphic [changes] each time):	要不然（可以使用共用的本地卷措施）你就能在docker里面使用wget直接下载（把文件移动进共用的文件夹）你得要会在匣子上apt-get update和apt-get install wget。好了我们运行Metasploit，msfconsole就可以了。你被问问题的话，第一个问题回答是（设置一个新的数据库）第二个回答否（[初始]web服务）。答那个问题之后，你应该看到[这样的]（实际的图形每次都[在变]）：	en	zh	2025-10-08 22:38:02	2	49
70	[The graphic changes each time you load it] so don't worry if your version doesn’t show [Missile] Command - [and in case] you [were wondering] - Missile Command was a very popular video game back [in the 1980's] :)	[你每次加载图形，图形都在变化]，所以你的版本上没有[投射物]命令（Missile Command）- [而以防万一]你[真想知道] - 《导弹指挥官》（同名）[早在20世纪80年代]是一个玩的人非常多的游戏 :)	en	zh	2025-10-08 22:55:10	2	50
71	We are going to run msfvenom from the Metasploit command line to generate two versions of meterpreter [executables]. The first will run an unmodified meterpreter executable:	我们打算在Metasploit命令行里运行msfvenom去生成两个版本的meterpreter[执行档]。第一个将运行一个未修改的meterpreter执行档：	en	zh	2025-10-08 22:55:24	2	51
72	You don't really have to worry about the parameters but if you are interested, the LHOST and LPORT arguments tell Meterpreter where to connect back to (i.e. our machine). We haven't set up a [listener] on port 4443 and our executable actually doesn't send back anything, so these values are just there [as a space holder]. The -f exe tells Meterpreter that we are using an executable format for the payload.	你不必真的担心变量，但你有兴趣的话，LHOST和LPORT变数就给Meterpreter说要连接回到的地方（换言之我们的机械）。我们并没有在端口4443上设置[侦听器]，而我们的执行档实际什么都没有发送回来，那么这些值只是在那[当一个空格位符]。-f exe给Meterpreter说，我们在给装载物使用一个可执行的格式。	en	zh	2025-10-09 08:34:08	2	52
73	Now we will create a second version that will effectively embed Meterpreter in a normal Windows executable putty.exe. Putty is an application that allows SSH connections on a Windows box. I have saved putty.exe in the /volume1 (shared) directory. Run the msfvenom command as follows:	好了我们将创建将有效地把Meterpreter嵌入到一个正常的Windows执行档putty.exe中的第二个版本。Putty是一个允许在Windows盒子里SSH连接的应用。我已经把putty.exe保存到/volume1（共用的）目录了。运行如下的msfvenom指令：	en	zh	2025-10-09 08:54:27	2	53
74	[Moreover], Msfvenom has other encoders that try and [obfuscate] the file to avoid detection. One of these is called shikata ga nai. You can create a meterpreter binary using the flag:	[此外]，Msfvenom有尽量[混淆]文件去规避探测的\r\n其他编码器。这些编码器其中一个叫做shikata ga nai。你可以使用旗子，就能创建一个meterpreter二进制：	en	zh	2025-10-09 08:54:51	2	54
75	Now that we have the three different samples, we can go back to our [classifier] in the ai-malware container and see what happens. [Launch] the ai-malware container with the volume1 attached (it will be a new container instance of the ai-malware image):	现在我们有三个不同的样本，我们在我们的ai-malware匣子中可以转回到我们的[分类器]并看情况。[启动]已附加voulme1的ai-malware匣子\r\n（将是ai-malware图像的一个新匣子实例）。	en	zh	2025-10-09 09:06:21	2	55
76	Now, move the meterpreter files into the /opt/ember directory. With the first version meterpreter1.exe we get:	好了，把meterpreter文件移动进/opt/ember目录。使用第一个版本meterpreter1.exe，我们得到：	en	zh	2025-10-09 09:18:44	2	56
77	[Definitely malware]! However, when we run the second version, we get:	[绝对是恶意软件]！不过我们运行第二个版本的时候，我们就得到：	en	zh	2025-10-09 09:26:01	2	57
78	So, according to the classifier, this is likely to be a normal, and safe, binary/executable! And [for] the last one:	所以根据分类器所述，这有可能是一个正常且安全的二进制/执行档！而[测]最后一个：	en	zh	2025-10-09 09:29:20	2	58
79	It is important to note that [machine learning evasion by masquerading as a normal binary] is not an adversarial technique. [You have simply overwhelmed the classifier with enough features of normal binaries] that it tips it into classifying it [as such]. Adversarial techniques in malware are more difficult than with images because you are more limited in [what you can change]. You still [want] a binary that works after your changes [and so] randomly changing bits of the file can easily [stop it from doing that].	注意到[机械学习伪装成一个普通二进制就可以回避]不是一个对抗技巧很重要。[你只不过是给分类器灌输足够多的普通二进制功能而已]，分类器才倒向把二进制[如此]分类。恶意软件中的对抗技术比用图像的难，是因为你比较受限于[你可以改的内容]。在你改动后你仍然[需要]一个能用的二进制，[因此]随机地改变文件的数位可以很容易[让他不那么做]。	en	zh	2025-10-09 10:28:20	2	59
80	Don't forget to delete all malware samples and also delete the local volume you created that contains the meterpreter files.	别忘记删除所有恶意样本并且删除你创建的、装meterpreter文件的本地卷。	en	zh	2025-10-09 10:29:51	2	60
81	If you are interested, upload the meterpreter versions you created to VirusTotal and see [what they are classified as there].	你有兴趣的话，把你创建了的meterpreter版本上传到VirusTool并看版本[在那分类成什么]。	en	zh	2025-10-09 10:31:24	2	61
82	It uses [single-pixel image] that steals your sensitive chat data and sends it to a malicious third-party. Full PDF-version	攻击使用窃取你敏感聊天数据的[单像素图像]并把图像发送给一个恶意第三方。完整PDF版本。	en	zh	2025-10-09 13:18:22	2	62
...
1	CITS1003_CyberSecurity_Forensics	\N
2	CITS1003 CyberSecurity Forensics	\N
3	PHL4100 The Singularity - A Philosophical Analysis	\N
4	PHIL4100 Ethics and Critical Thinking - The Singularity - A Philosophical Analysis	\N
5	CITS1402 Relational Database Management - Lab 6 - Dental Clinic	\N
6	OALD7-E&C	\N
7	MyBatch	\N
8	CITS 1003 Cybersecurity - Lab 10 - AI	\N
9	CITS 1401 Python - Lab 6 - Dictionaries and classes/objects	\N
10	CITS 1401 Python - Lab 7 - Project 2 - XYZ Research Institute	\N
11	string	\N
\.
...
SELECT pg_catalog.setval('public.entries_id_seq', 139, true);
...
SELECT pg_catalog.setval('public.sources_id_seq', 12, false);
...
--
-- Name: entry_sources entry_sources_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.entry_sources
    ADD CONSTRAINT entry_sources_pkey PRIMARY KEY (entry_id, source_id);


--
-- Name: entry_sources entry_sources_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.entry_sources
    ADD CONSTRAINT entry_sources_unique UNIQUE (entry_id, source_id);
...
--
-- Name: idx_entries_text; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_entries_text ON public.entries USING btree (src_text, tgt_text);


--
-- Name: ix_articles_created_at; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: entry_sources entry_sources_entry_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.entry_sources
    ADD CONSTRAINT entry_sources_entry_id_fkey FOREIGN KEY (entry_id) REFERENCES public.entries(id);


--
-- Name: entry_sources entry_sources_source_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.entry_sources
    ADD CONSTRAINT entry_sources_source_id_fkey FOREIGN KEY (source_id) REFERENCES public.sources(id);
...
\unrestrict JhgQfJFgd016X2rsj5V8FQOTOwAcxh0ujlJUWZfLPwJ8c8P95TRRHnL0PvRbMKU

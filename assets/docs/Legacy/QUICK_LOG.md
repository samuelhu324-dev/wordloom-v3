...
7. D21/Orbit/Feature: 添加一个note复制功能，并自动生成一个新id和其图片的后台管理文件夹！但是使用次数重置！
8. D21/Orbit/Feature: Note必须提供一个自动插入表格的功能，不然一些汇总类的note看着非常眼睛不舒服！以及序号像1. 2. 这样的可以自动变为一类颜色。
9. D21/Orbit/Feature: 要预防各种一开就破的情况： $env:PYTHONPATH="D:\Project\Wordloom\WordloomBackend\api" ; python -m uvicorn app.main_orbit:app --reload --host 127.0.0.1 --port 8000
10. D21/--/--: 各种404和500有可能是功能不完善本身导致的问题（比如缺少导入的组件、莫名其妙出现的变量，错位的端口等）也有可能是项目批次不一样导致生成内容产生了差异。
11. D21/--/--：啃代码要从UI相关的设计开始，再啃书到最基础的代码，层层递进，先从表层的门面开始；不过搭建内容要从功能开始才开始改进UI，不然可能会出一些问题；
12. D21/Orbit/Feature-bookshelf: 这个功能涉及到文件存储和清理的问题，以及嵌套创建！

## 2025-11-02
1. D23/Orbit/Feature-Notes: 每次的note内容都应该备份，不然每次修改都有一定概率会损失原本的内容；以及测试的时候尽可能采用测试文档，不要采用原本的文档来进行测试，以免文档内容丢失！（经验教训）13:06
2. D23/Orbit/Feature-Notes: 除开自己的文档以外，希望每个note的block内容有一个备份文档储存在一个文件夹里面，这样就不会出现哪怕用了原本文档，文档会丢失的情况；13:08
3. D23/Orbit/Feature-Notes: 关于目前调整block后，对checkpoint功能的改造要点：（1 首先不需要"检查点11:26:57"这种命名，应该可以在创建时由用户自己命名；（2 创建时间应该按照当前的系统时间来进行设置，而创建时间也可以进行编辑，检查点在右上角应该可以有一个鼠标悬停后可以找到编辑按钮进行修改的地方：主要可以修改标题（就是截图1中检查点11：26：57这个位置）和创建时间；（3 而截图1中代办状态应该在完成度变为100%以后变成”完成“而没有100%时为“待办”；（4 而代办和完成度这一栏也应该变成一个鼠标悬停后可以查看的内容跑到右上角，和检查点的“编辑、删除”放在一起都在右上角,不需要那个“pending”提示；（5 每一个marker（标记点，即完成记录：标题一行、但是内容为另一行，并且就像文本内容一样，随时都为编辑状态，不需要特地点击编辑才能修改）；（5 所以说marker其实不需要右上角鼠标悬停以后那个编辑按钮，应该可以直接编辑；（6 而时间的编辑这种模式不太好，我希望是有固定格式，可以直接打字修改，不用去查看复杂的浏览日期界面的方式；
4. D23/Orbit/Feature-Notes: 现在的检查点（1 截图1：首先不需要前面的📋这个emoji，直接去掉，还有就是字体太大，改小一点；（2 截图2：创建时间出来以后不是我本机目前显示出来的时间！；（3 截图3：另外现在检查点不需要右上角的删除和编辑了，现在检查点和时间都应该可以眼睛看到即所得的进行修改，不需要额外再去点击按钮；（4 截图3：整改鼠标选厅查看完成度中间这一行的内容，我都希望跑到现在即将不需要的“编辑和删除那个位置”，只有鼠标放到那里才可以看到完成度和数据；（5 你可以修改吗？
5. D23/Orbit/Feature-Notes:
直接打字叫做inline编辑；时间分为各种方法，典型的是用本地时间计数

（1 关于主题颜色修改：眼睛快要痛死了，每天看白色 ；以及板块中间有一一大块灰色底色（2 BlockManager的浮动显示，除开工具栏始终浮动和粘性以外，每个Checkpoint标题切换到下一个block之前也应该是浮动的（因为不是每个block都有标题）；（3 现在marker之间的顺序会乱跳，设置成拖拽。（4 没有及时保存到Git，爽不爽？舒服了把。
